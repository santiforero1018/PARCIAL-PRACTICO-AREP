...
public class Calculator {
    ...
    private static void getAndIvokeTheMethod(String operation, String[] Parameters) {
        try {
            Method method = math.getMethod(operation, Double.TYPE);
            if(method.getParameters().length == 1){
                method.invoke(null,  Double.parseDouble("2.3"));
            } else {
                method.invoke(null, Double.parseDouble("2.3"), Double.parseDouble("2.3"));
            }
        } catch (Exception e) {
